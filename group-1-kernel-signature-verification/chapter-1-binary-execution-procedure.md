...
上面的链表体现了典型的 Intel 64-bit 机器中，内核默认编译配置下，识别一个二进制文件格式的次序。哪些因素决定了内核以不同的优先级依次识别各个二进制文件格式呢？
...
解决方案的核心思想是，实现一个伪 ELF 处理模块 \(`binfmt_elf_signature_verification`\)，并通过对上述三个因素的控制，将该模块注册在链表中 ELF 处理模块的前面。在该模块中，实现签名验证的逻辑。如果签名验证通过，则返回 `-ENOEXEC` 错误码，使得内核继续调用真正的 ELF 处理模块；如果签名验证不通过，则直接返回其它错误码，使得内核不再调用真正的 ELF 处理模块。
...
我们提供了两种可选的方案，前者侧重性能，后者侧重灵活性：

1. 将伪 ELF 处理模块编译进内核主映像
2. 将伪 ELF 处理模块编译为可挂载的内核模块

#### 将处理模块编译到内核主映像

这个解决方案不需要修改内核中已有的 C 代码，因此能够轻松扩展到更高版本的内核。只需要在编译配置中加入一个新的编译选项，就能将我们实现的 C 文件编译进内核中：

* 在 `fs/Kconfig.binfmt` 中加入一个新的编译选项 `BINFMT_ELF_SV`
* 在 `fs/Makefile` 中加入一行编译命令 `obj-$(CONFIG_BINFMT_ELF_SV) += binfmt_elf_signature_verification.o`

通过对上述[内核进行二进制模块初始化的顺序](chapter-1-binary-execution-procedure.md#14-nei-he-shi-bie-er-jin-zhi-wen-jian-ge-shi-de-ci-xu)进行妥善控制，可以将处理模块插入到如图所示的位置 \(图中 `binfmt_elf_sv` 是 `binfmt_elf_signature_verification` 的简写\)。这个方案的粒度较细，对其它非 ELF 格式的二进制文件的执行性能没有影响，因此在理论上的效率是最高的。

![&#x5904;&#x7406;&#x6A21;&#x5757;&#x88AB;&#x7F16;&#x8BD1;&#x8FDB;&#x5185;&#x6838;&#x4E3B;&#x6620;&#x50CF;&#x4E4B;&#x540E;&#xFF0C;&#x5728;&#x5904;&#x7406;&#x94FE;&#x8868;&#x4E2D;&#x7684;&#x4F4D;&#x7F6E;](../.gitbook/assets/binfmt-elf-sv-kernel.png)

#### 将处理模块编译为可挂载的内核模块

这种方法的灵活性较强，用户可以在内核启动后，通过 `insmod` 命令动态加载这个模块，通过 `rmmod` 命令动态卸载这个模块。与前一种方案的区别在于，当我们准备挂载该模块时，内核中的二进制处理模块链表已经初始化完成了。由于模块被插入链表的方式只有从链表头插入和从链表尾插入，而插入到链表尾部使得模块无法在系统内置的 `binfmt_elf` 模块之前被执行，因此只能在链表的头部插入我们的处理模块：

![&#x5904;&#x7406;&#x6A21;&#x5757;&#x88AB;&#x7F16;&#x8BD1;&#x4E3A;&#x5185;&#x6838;&#x6A21;&#x5757;&#x4E4B;&#x540E;&#xFF0C;&#x5728;&#x5904;&#x7406;&#x94FE;&#x8868;&#x4E2D;&#x7684;&#x4F4D;&#x7F6E;](../.gitbook/assets/binfmt-elf-sv-module.png)

这种方案与前一种相比，理论上会有一些额外的开销。因为内核执行所有的二进制文件时，都会经过我们实现的处理模块。但是非 ELF 格式的二进制文件将无法通过 `binfmt_elf_signature_verification` 模块的 ELF 格式检查，从而返回 `-ENOEXEC`，交由之后其它的二进制文件处理模块进行处理。
