...
当我们在 shell 中输入命令，试图执行一个二进制文件时，shell 会调用 Linux 内核的 `execve` 系统调用。该系统调用会将二进制文件部分装载进内存，判断其格式是否合法、是否具有执行权限。如果一切正常，则将当前进程的代码段、数据段替换为新程序，并设置好命令行参数、环境变量、新程序的入口地址，完成新程序开始运行前的一切准备工作。

如果 `execve` 的执行一切正常，那么原进程的运行环境将会被新进程完全替换。对于 shell 的使用者来说，一定希望能够在运行完一条命令后再次回到 shell 中继续输入下一条命令。为了避免 shell 进程在调用 `execve` 之后被新程序覆盖，在 shell 的实现中，通常会通过 `fork` 系统调用，先复制出一个 shell 进程的副本，然后在副本进程中调用 `execve`。执行结束后，回到 shell 主进程中，用户可以继续执行下一个程序。

![Shell &#x6267;&#x884C;&#x7A0B;&#x5E8F;&#x7684;&#x6D41;&#x7A0B;](../.gitbook/assets/shell.png)

## 1.2 识别不同格式的二进制文件

内核支持执行多种格式的二进制文件。比如一个 ELF 文件，或是一个 shell script，甚至还可以是一幅图片、一个 PDF 文档。对于不同的二进制文件格式，内核需要为其准备不同的运行环境。比如：

* 对于一个 ELF 文件，内核需要找到该文件中的代码段、数据段，以及程序的入口地址，并将这些信息设置在进程控制块中；如果该文件需要动态链接，那么还要处理动态链接的相关信息
* 对于一个 shell 脚本，内核需要寻找文件中指定的 **脚本解释器**，并将脚本文件作为参数启动脚本解释器，解释执行这个脚本
* 对于一个 PDF 文档或图片，内核需要寻找相应的 PDF 解释器或图片解释器，并将文件作为解释器参数，启动解释器

为了识别二进制文件的格式，内核会将二进制文件的前 128 个字节装入内存中如下的结构体中：
...
内核根据文件前 128 字节中的 magic value \(每个格式特有的字节序列\) 等信息，识别出二进制文件的格式。比如：
...
在 Linux 4.15.0 中，内核默认能够执行哪些格式的二进制文件呢？在该版本的内核源代码中，已经内置了部分二进制文件格式的处理程序 \(handler\)，位于内核代码的 `fs/` 目录下：
...
其中，每一种格式对应的处理程序，在编码方式上都以模块的形式实现 \(虽然编译后并不一定是模块\)，并且都以多态的思想实现了一个统一的接口：
...
在操作系统初始化阶段，这些二进制处理函数被依次添加到了一个链表上。当执行一个二进制文件时，内核实现将其头 128 字节以及相关信息读入 `struct linux_binprm` 结构体，然后依次遍历链表上的每一个 `struct linux_binfmt` 结构体，以 `struct linux_binprm` 作为参数，依次调用每种二进制格式对应的 `*load_binary` 函数。这一过程在 `fs/exec.c` 的如下函数中实现：
...
1. 当前处理函数不能识别这个二进制文件的格式 - 那么返回 `-ENOEXEC` 的错误码，继续尝试调用下一个格式的处理函数识别文件的格式
2. 当前处理函数成功识别了这个二进制文件的格式，并顺利完成了执行这个二进制文件的准备工作，`execve` 系统调用正确返回
3. 当前处理函数成功识别了这个二进制文件的格式，但在执行该文件准备工作的过程中发生了错误 \(比如动态分配内存失败，或该文件的内容损坏/不一致\)，则将相应错误码返回 `execve` 系统调用并停止遍历链表。最终 `execve` 系统调用将错误码返回到 shell，向用户提示运行失败的原因

以内核执行 ELF 文件为例，内核搜索 ELF 处理函数的过程如下图所示。

1. 内核首先调用 `binfmt_misc` 模块中的 `load_binary()` 函数，对文件的前 128 字节进行识别，发现该文件不属于 misc 类型，返回 `-ENOEXEC`
2. 内核继续调用 `binfmt_script` 模块中的 `load_binary()` 函数，发现该文件也不是一个脚本，返回 `-ENOEXEC`
3. 内核继续调用 `binfmt_elf` 模块中的 `load_binary()` 函数，发现该文件是一个 ELF 文件；那么接下来开始根据 ELF 的格式解析这个文件，直到最后成功执行这个程序

![&#x4E8C;&#x8FDB;&#x5236;&#x6587;&#x4EF6;&#x5904;&#x7406;&#x51FD;&#x6570;&#x94FE;&#x8868;](../.gitbook/assets/search-binary-handler.png)

## 1.4 内核识别二进制文件格式的次序

上面的链表体现了 Intel 架构 64-bit 机器上，在内核默认编译配置下，识别一个二进制文件格式的次序。哪些因素决定了内核以不同的优先级依次识别各个二进制文件格式呢？

内核在 **初始化阶段** 逐步构造二进制文件处理函数的链表。每个格式的处理函数被放入链表的时机和方式由如下因素决定：

1. 二进制格式处理模块在内核中的初始化等级
2. 二进制格式处理模块被编译的顺序
3. 二进制格式处理模块被插入链表的方式 \(在链表头部插入 / 在链表尾部插入\)
...
每种格式的二进制文件处理模块中都定义了初始化函数。在该函数中，该模块将其自身注册到内核的二进制文件处理函数链表上。内核提供两种注册二进制文件处理模块的方式：
...
本解决方案的目标是，在内核为执行一个 ELF 文件进行原有的准备工作之前，先对 ELF 中的签名进行验证。如果验证通过，则继续进行准备工作；如果验证失败，则内核直接停止后续工作，拒绝执行这个 ELF 文件。

解决方案的核心思想是，以 [二进制文件处理程序](chapter-1-binary-execution-procedure.md#13-er-jin-zhi-wen-jian-ge-shi-chu-li-cheng-xu-binary-format-handler) 的形式，实现一个 ELF 签名验证模块 \(`binfmt_elf_signature_verification`\)，并将该模块注册到链表中 ELF 处理模块 \(`binfmt_elf`\) 之前。在该模块中，实现签名验证的逻辑。如果签名验证通过，则返回 `-ENOEXEC` 错误码，使得内核能够继续遍历链表，直到调用真正的 ELF 处理模块；如果签名验证不通过，则直接返回其它错误码，使得内核不再继续调用真正的 ELF 处理模块。

在这个 ELF 签名验证模块中，需要进行的工作主要有两点：

1. 格式检查 - 判断该二进制文件是否符合 ELF 格式，跳过所有非 ELF 格式的文件
2. 签名验证 - 如果文件符合 ELF 格式，则按 ELF 格式取出其中附带的数字签名并验证
