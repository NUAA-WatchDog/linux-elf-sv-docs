...
解决方案的核心思想是，实现一个伪 ELF 处理模块 \(binfmt\_elf\_signature\_verification\)，并通过对上述三个因素的控制，将该模块注册在链表中 ELF 处理模块的前面。在该模块中，实现签名验证的逻辑。如果签名验证通过，则返回 `-ENOEXEC` 错误码，使得内核继续调用真正的 ELF 处理模块；如果签名验证不通过，则直接返回其它错误码，使得内核不再调用真正的 ELF 处理模块。大致过程如下图所示 \(图中 binfmt\_elf\_sv 是 binfmt\_elf\_signature\_verification 的简写\)：

![binfmt\_elf\_signature\_verification &#x6A21;&#x5757;&#x5728;&#x94FE;&#x8868;&#x4E2D;&#x7684;&#x4F4D;&#x7F6E;](../.gitbook/assets/binfmt-elf-sv.png)

这个解决方案不需要修改内核中已有的 C 代码，因此能够轻松扩展到更高版本的内核中。另外，这个方案的粒度较细，对其它非 ELF 格式的二进制文件的执行性能没有影响，因此效率较高。
